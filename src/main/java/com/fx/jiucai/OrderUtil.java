...
//        	System.out.println("等待初始化中");
        ...
        while(true) {
	        try {
	    	    ...
	    	    // ===========           下单         ==============
	    	        Connection.Response createOrderResponse = Jsoup.connect("https://mall.phicomm.com/order-create-is_fastbuy.html").method(Connection.Method.POST).timeout(SystemConstant.TIME_OUT).ignoreContentType(true)
	    	            .cookies(cookies)
