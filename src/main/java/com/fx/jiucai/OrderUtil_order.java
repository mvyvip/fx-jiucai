...
public class OrderUtil_order {

	private static volatile String rsbody = null;

	public static void main(String[] args) throws Exception {
		Map<String, String> cookies = getCookies("18774439564", "li5201314");
//        initBody(cookies);
...
//		Document document = Jsoup.parse(Jsoup.connect("https://mall.phicomm.com/cart-fastbuy-12-1.html").method(Connection.Method.GET).timeout(SystemConstant.TIME_OUT).cookies(cookies).followRedirects(true).execute().body());
		Document document = Jsoup.parse(Jsoup.connect("https://mall.phicomm.com/cart-fastbuy-197-1.html").method(Connection.Method.GET).timeout(SystemConstant.TIME_OUT).cookies(cookies).followRedirects(true).execute().body());
		if(document.body().text().contains("立即购买失败")) {
			...
			new Thread(new Runnable() {
				...
				public void run() {
					try {
				        ...
				        if(body.contains("返回购物车修改")) {
				        	updateRsBody(body);
				        } else if(body.contains("库存不足")) {
				        	System.out.println("库存不足 - " + new Date().toLocaleString());
				        } else if(body.contains("购物车为空")) {
...
//                    System.err.println("addr_id获取成功："  + addr_id);
    ...
    private static Map<String, String> getCookies(String username, String password) throws Exception {
        Response pageResponse = Jsoup.connect("https://mall.phicomm.com/passport-login.html").method(Connection.Method.GET).execute();
